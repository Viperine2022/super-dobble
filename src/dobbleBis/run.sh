...
case $1 in
  ...
  5) # 1 : Génération du code java pour affecter les cartes déterminées au modèle
     # 2 : Puis mise à jour de la variable NB_CARTES_DEJA_GENEREES dans le fichier DobbleBis.java
  output=$(./run.sh 1 $nbCartes | awk '{print "v[" $1 "][" $2 "],"}') && echo -n "solver.add(context.mkAnd(${output%?}));" > outputYES.txt && addCards=$(cat outputYES.txt) && sed -i "/.*Determiner les N cartes du modele.*/a $(echo $addCards)" DobbleBis.java && sed -i "s/.*public final int NB_CARTES_DEJA_GENEREES.*/    public final int NB_CARTES_DEJA_GENEREES = ${nbCartes};/" DobbleBis.java;;

  6) # Nier la solution en cours
  output=$(./run.sh 1 $nbCartes | awk '{print "v[" $1 "][" $2 "],"}') && echo -n "solver.add(context.mkNot(context.mkAnd(${output%?})));" > outputNOT.txt && addLines=$(cat outputNOT.txt) && sed -i "/.*Nier la solution en cours pour en obtenir une nouvelle.*/a $(echo $addLines)" DobbleBis.java;;

  7) # Reset le jeu Dobble <=> NB_CARTES_DEJA_GENEREES = 0 et pas de solution niée
  sed -i "s/.*public final int NB_CARTES_DEJA_GENEREES.*/    public final int NB_CARTES_DEJA_GENEREES = 0;/" DobbleBis.java;;
