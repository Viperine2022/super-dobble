...
if [ "$#" -ne 2 ] || [ "$1" -gt "$2" ]; then
  echo -e "\nUsage : ./while.sh n1 n2      avec n1 < n2 et n1 >= 20\nPar exemple, ./run.sh 20 25\n"
  exit 1
fi
...
do
  ...
  numMaxMethode=$(cat DobbleBis.java | grep "Errones" | cut -d "(" -f 1 | cut -d "s" -f 3 | sort -n | tail -n 1)
  ...
  if [ "${i}" -ge 25 ]; then
    ...
    sed -i "/.*Determiner les N cartes du modele.*/a $(echo "public void ajoutModelesErrones$((${numMaxMethode}+1))\(\) {")" DobbleBis.java
    sed -i "/.*Determiner les N cartes du modele.*/a $(echo })" DobbleBis.java

    # On ajoute le code de l'appel de la méthode qu'on vient de créer
    sed -i "/.*Dobble dobble.*/a $(echo "dobble.ajoutModelesErrones$((${numMaxMethode}+1))\(\)\;")" DobbleBis.java
  ...
  # ON N'A PAS PU GENERER UN JEU DE $2 CARTES ( SAT_PROBLEM = 1 )
    ...
    lineToAdd=$(($(grep -n "Determiner les N" DobbleBis.java | cut -d ":" -f 1)+1))

    # Nier le modèle qui vient d'être généré (on ajoute le code java correspondant)
    replace=$(sed -n ${lineToAdd}p DobbleBis.java | awk -F "(" '{print $1 "(context.mkNot(" $2 "(" $3 }' | head -n 1 | awk -F ")" '{print $1 ")))" $2 $3 }')
    sed -i "${lineToAdd}s/^.*$/${replace}/" DobbleBis.java
